...
const UploadCaseFiles = (props) => {
  ...
  const handleSave = async () => {
    ...
    if (caseFiles)
      caseFiles.map((val, index) => {
        if (val.isApproved) {
          var tmpCaseFiles;
          firestore().collection('Cases').doc(patientID).collection('Case').get().then((query) => {
            query.forEach((doc) => {
              tmpCaseFiles = doc.data().CaseFiles;
              tmpCaseFiles.forEach((value) => {
                if (val.label != value.label)
                  firestore().collection('Cases').doc(patientID).collection('Case').doc(caseID).update({ CaseFiles: [...tmpCaseFiles, val] });
              })
            })
          })
        }
