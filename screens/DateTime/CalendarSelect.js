  ...
  renderDay(day) {
    ...
    return (
      <TouchableOpacity onPress={() => this.selectDate(day)}
        ...
        dateSelected ? { backgroundColor: '#0064FE', dateSelectedWarpDayStyle, borderRadius: 50 } : {}]}
      >
        <View>
          {renderChildDay(day)}
          <Text style={[
          styles.day, textDayStyle,
          isCurrent ? { color: 'red', ...currentDayStyle } : {},
          notCurrentMonth ? { color: '#493D40', ...notCurrentDayOfMonthStyle } : {},
          dateSelected ? {color:"white"} : {}
          ]}>
            {day.split('-')[2]}
          </Text>
        </View>
      ...
      <View style={styles.container}>
        <View style={[{ flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center' }, warpRowControlMonthYear]}>
          <TouchableOpacity onPress={() => this.yearMonthChange(-1, 'year')}>
            {renderPrevYearButton ? renderPrevYearButton() : <MCIcons name='chevron-double-left' size={30} color='#AAF' />}
          </TouchableOpacity>
          <TouchableOpacity onPress={() => this.yearMonthChange(-1, 'month')}>
            {renderPrevMonthButton ? renderPrevMonthButton() : <MCIcons name='chevron-left' size={30} color='#AAF' />}
          </TouchableOpacity>
          <Text style={styles.txtHeaderDate}>{calendarMonth.split('-')[1]}</Text>
          <Text style={styles.txtHeaderDate}>{calendarMonth.split('-')[0]}</Text>
          <TouchableOpacity onPress={() => this.yearMonthChange(1, 'month')}>
            {renderNextYearButton ? renderNextYearButton() : <MCIcons name='chevron-right' size={30} color='#AAF' />}
          </TouchableOpacity>
          <TouchableOpacity onPress={() => this.yearMonthChange(1, 'year')}>
            {renderNextMonthButton ? renderNextMonthButton() : <MCIcons name='chevron-double-right' size={30} color='#AAF' />}
          </TouchableOpacity>
        </View>
